...
import { useAuthStore, useCartStore, useThemeStore } from '../../utils/store';
    ...
    <nav className="sticky top-0 z-40 bg-white/90 dark:bg-gray-900/90 backdrop-blur border-b border-gray-100 dark:border-gray-800">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {/* Dark mode toggle */}
          <button onClick={toggle} className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 text-lg transition-colors" title="Toggle dark mode">
            {dark ? '☀️' : '🌙'}
          </button>
          ...
          {user ? (
            <div className="relative">
              ...
              {dropOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 py-1 z-50"
                  ...
                  {user.role === 'admin' && (
                    <Link to="/admin" className="block px-4 py-2.5 text-sm text-orange-500 font-medium hover:bg-orange-50 dark:hover:bg-gray-700" onClick={() => setDropOpen(false)}>⚙️ Admin Panel</Link>
                  )}
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden px-4 pb-4 space-y-2 border-t dark:border-gray-800 pt-3">
