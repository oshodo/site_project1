...
const CATEGORIES = [
  'Electronics', 'Computers', 'Footwear', 'Clothing',
  'Home & Kitchen', 'Sports', 'Books', 'Accessories',
]
    ...
    <header className="bg-gray-900 sticky top-0 z-50 shadow-md">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
        ...
        <Link to="/" className="text-2xl font-bold text-orange-500 flex-shrink-0">
          Sabai<span className="text-white">Sale</span>
        </Link>
            ...
            <div className="relative">
              ...
              {dropdown && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50">
                  <p className="px-4 py-2 text-xs text-gray-500 border-b truncate">{user.email}</p>
                  ...
                  {user.role === 'admin' && (
                    <Link to="/admin" onClick={() => setDropdown(false)}
                      className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-50 text-orange-600 font-medium">
                      <FiSettings size={14} /> Admin Panel
                    </Link>
                  )}
                  <hr className="my-1" />
                  <button onClick={() => { logout(); setDropdown(false) }}
                    className="flex items-center gap-2 w-full px-4 py-2 text-sm hover:bg-gray-50 text-red-500">
        ...
        <div className="md:hidden bg-gray-800 px-4 pb-4 space-y-3">
          <form onSubmit={handleSearch} className="flex mt-3">
            <input value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search..." className="w-full px-3 py-2 rounded-l-lg text-sm text-gray-900 outline-none" />
